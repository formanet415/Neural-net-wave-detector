...
##      Function for editing labels     ## requires manual prompting
...
## SAVING REST OF LABELED FILE (Function) ## Runs on its own

def save_unlabeled(cdf,txt,limit):
    nsamp = cdf['SAMPS_PER_CH']
    lngth = 4096
    for i in range(limit):
        if i not in txt:
            ...
            snap_label = np.zeros(parts).astype(int)
            if 'out_label' not in locals():
                out_label = snap_label
            else:
                out_label = np.append(out_label, snap_label)
    ...
    with open('empty_rswf_data.pickle', 'wb') as f:
        pickle.dump(out_data, f)

    with open('empty_rswf_label.pickle', 'wb') as f:
        pickle.dump(out_label, f)

    print('saved')

save_unlabeled(esw_rswf, resw_txt)
