...
def reshape(x):
    ...
    for i in range(len(x[0, :, 0])):
        newx[i, :, 0] = np.reshape(x[0, i, :],-1)
        newx[i, :, 1] = np.reshape(x[1, i, :], -1)
    return(newx)
...
x = layers.Conv1D(32, 128, padding='valid', activation='relu')(inputs)
x = layers.MaxPooling1D(pool_size=2)(x)
x = layers.Conv1D(64, 64, activation='relu')(x)
x = layers.Flatten()(x)
...
model.fit(x_train, y_train, batch_size=300, epochs=75, verbose=2)
model.evaluate(x_validate, y_validate, batch_size=10, verbose=2)

print("Empty to non empty training ratio is: %f" %((np.sum(y_train==0))/(y_train.shape[0])))
print("Empty to non empty validate ratio is: %f" %((np.sum(y_validate==0))/(y_validate.shape[0])))


model.save('time_series_CNN_simple/')
